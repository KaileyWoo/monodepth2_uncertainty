...
class MyMonoDataset(data.Dataset):
    ...
    def __init__(self,
                 ...
                 is_train=False,
                 img_ext='.jpg'):
        super(MyMonoDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        # side为l或r，表明该图片是左或右摄像头所拍。这里只有一个摄像头，这个值一直为2
        if len(line) == 3:
            side = line[2]
        else:
            side = None
        ...
        for i in self.frame_idxs:
            if i == "s":
                ...
                inputs[("color", i, -1)] = self.get_color(folder, frame_index + i + i, side, do_flip)
