...
class Trainer:
    def __init__(self, options):
       ...
       # self.nets = 8
        #self.load_boots_models()   # load boots net
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            uncert_E = torch.exp(outputs[("dropout_uncert", 0)])
            uncert_A = torch.exp(outputs[("uncert", 0)])
            for frame_id in self.opt.frame_ids[1:]:
                # 按尺度获得对应图像的预测图（即深度图转换到点云再转到二维图像最后采样得到的彩图
                pred = outputs[("color", frame_id, scale)]
                reprojection_loss = self.compute_reprojection_loss(pred, target)
                uncertA_loss=reprojection_loss/(uncert_A + 1e-10) +  uncert_A
                uncertE_loss=reprojection_loss/(uncert_E + 1e-10) +  uncert_E
                reprojection_losses.append(reprojection_loss)
                reprojection_losses.append(uncertA_loss)
                reprojection_losses.append(uncertE_loss)
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                if s==0:
                    writer.add_image("uncert_A_{}/{}".format(s, j),normalize_image(outputs[("uncert", 0)][j]), self.step)         
                    writer.add_image("uncert_E_{}/{}".format(s, j),normalize_image(outputs[("dropout_uncert", 0)][j]), self.step) 
    ...
    def load_depth_pose_models(self):
        print("-------------------------------------Load depth & pose model!*****WSY*****")
    ...
    def load_data(self):
        ...
        train_filenames = readlines(fpath.format("train"), False)
        val_filenames = readlines(fpath.format("val"), False)
