...
class Trainer:
    def __init__(self, options):
        ...
        #self.load_T_model()   # load teacher net
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                self.models["pose_encoder"] = networks.ResnetEncoder(self.opt.num_layers,False,num_input_images=self.num_pose_frames)
        ...
        if self.opt.predictive_mask:
            ...
            self.models["predictive_mask"] = networks.DepthDecoder(self.models["encoder"].num_ch_enc, self.opt.scales,num_output_channels=(len(self.opt.frame_ids) - 1))
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            #T_features = self.T_models["encoder"](inputs["color_aug", 0, 0])
            #T_outputs = self.T_models["depth"](T_features)
            ...
            for i in range(6):
                features = self.models["encoder"](inputs["color_aug", 0, 0])
                outputs = self.models["depth"](features)
                features_distribution.append( torch.unsqueeze(features[-1],0) )
            ...
            features[-1] = torch.mean( torch.cat(features_distribution, 0), dim=0, keepdim=False)
            ...
            '''disps_distribution = []
            for i in range(8):
                T_outputs = self.T_models["depth"](self.T_models["encoder"](inputs["color_aug", 0, 0]))
                disps_distribution.append( torch.unsqueeze(T_outputs[("disp", 0)],0) )
            # uncertainty as variance of the predictions
            pred_uncert = torch.var(torch.cat(disps_distribution, 0), dim=0, keepdim=False)'''
  
            #pred_uncert = torch.exp(T_outputs[("uncert", 0)])
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            #uncerts = outputs[("dropout_uncert", 0)]
            #target = T_outputs[("color", 0)]
            uncerts = torch.exp(outputs[("uncert", source_scale)])
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                reprojection_losses.append(reprojection_loss/(uncerts + 1e-10) +  uncerts)
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            ...
            # 不确定性图
            writer.add_image("uncert_0/{}".format(j),normalize_image(outputs[("uncert", 0)][j]), self.step)
    ...
    def load_T_model(self):
        print("   Loading pretrained teacher encoder")
        self.T_models["encoder"] = networks.ResnetEncoder(18, False)
        loaded_dict_enc = torch.load("./tmp/mono_model_519/models/weights_19/encoder.pth", map_location=self.device)
        filtered_dict_enc = {k: v for k, v in loaded_dict_enc.items() if k in self.T_models["encoder"].state_dict()}
        self.T_models["encoder"].load_state_dict(filtered_dict_enc)
        self.T_models["encoder"].to(self.device)
        self.T_models["encoder"].eval()

        print("   Loading pretrained teacher decoder")
        self.T_models["depth"] = networks.DepthUncertaintyDecoder(num_ch_enc=self.T_models["encoder"].num_ch_enc, scales=range(4), uncert=True, dropout=True)
        loaded_dict = torch.load("./tmp/mono_model_519/models/weights_19/depth.pth", map_location=self.device)
        self.T_models["depth"].load_state_dict(loaded_dict)
        self.T_models["depth"].to(self.device)
        self.T_models["depth"].eval()
